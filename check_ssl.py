...
def verificar_certificado_ssl(url):
    ...
    try:
        ...
        if data_atual < data_expiracao:
            ...
            return f"[OK] Certificado SSL de '{url}' é válido até {data_expiracao.strftime('%Y-%m-%d %H:%M:%S %Z')} - Dias para expirar: {dias_expirar}"
        else:
            return f"[ERRO] Certificado SSL de '{url}' expirou em {data_expiracao.strftime('%Y-%m-%d %H:%M:%S %Z')}."
